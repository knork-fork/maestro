...
1. Read the discuss artifact at `resources/tickets/{{ticket_id}}/discuss.md`. It defines the audit scope — which files, endpoints, flows, or components to focus on. Do not audit beyond what is described there.
...
1. Write a structured audit report (findings by severity, overall risk assessment) to `resources/tickets/{{ticket_id}}/audit.md`.
2. Read `resources/ticket-state.json`, set `status` for this ticket to `"audited"`, and write the file back.
